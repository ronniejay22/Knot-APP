    ...
    try:
        ...
        if milestone_context is None:
            ...
            if error:
                ...
                if final_three:
                    # Shares POST /generate's row builder, so rows read back
                    # via the by-milestone endpoint render at full fidelity:
                    # guaranteed image, personalization note, and idea content
                    # sections. This used to be a hand-copied duplicate, which
                    # is how the mixed-batch `is_idea` bug reached three call
                    # sites at once.
                    rec_rows = [
