...
| File | Purpose |
|------|---------|
| `KnotApp.swift` | Main app entry point. Configures SwiftData ModelContainer and injects it into the app environment. **Step 7.4:** Added `@UIApplicationDelegateAdaptor(AppDelegate.self) var appDelegate` to bridge UIKit delegate callbacks into the SwiftUI lifecycle. **Step 9.1:** Added `@State private var deepLinkHandler = DeepLinkHandler()`, injects it via `.environment(deepLinkHandler)` on ContentView, and handles incoming Universal Links via `.onOpenURL { url in deepLinkHandler.handleURL(url) }`. |
| `ContentView.swift` | Root auth state router. Creates `AuthViewModel` as `@State`, injects it into the SwiftUI environment via `.environment()`, and starts the `authStateChanges` listener via `.task`. Routes between three states: loading spinner (`isCheckingSession`), `HomeView` (`isAuthenticated`), or `SignInView` (default). **Step 9.1:** Added `@Environment(DeepLinkHandler.self) private var deepLinkHandler` for deep link navigation readiness (consumed in Step 9.2). |
...
- **External APIs:** `YELP_API_KEY` (Step 8.1), `TICKETMASTER_API_KEY` (Step 8.2), `AMAZON_ACCESS_KEY`/`AMAZON_SECRET_KEY`/`AMAZON_ASSOCIATE_TAG` (Step 8.3), `SHOPIFY_STOREFRONT_TOKEN`/`SHOPIFY_STORE_DOMAIN` (Step 8.4), `OPENTABLE_AFFILIATE_ID` (Step 8.5, optional), `FIRECRAWL_API_KEY` (Step 8.6)
