...
| `DeepLinkHandler.swift` | **Active (Step 9.1).** `@Observable @MainActor` class managing deep link state for Universal Links. Defines `DeepLinkDestination` enum (currently `.recommendation(id: String)`, extensible for future deep link types). `handleURL(_:)` parses incoming Universal Link URLs by splitting the path into components and matching against known patterns (`/recommendation/{id}`). Sets `pendingDestination: DeepLinkDestination?` which consuming views observe to trigger navigation (Step 9.2). Unrecognized paths are silently ignored (logged to console). Created as `@State` in `KnotApp` and injected into the SwiftUI environment. Follows the same `@Observable @MainActor` pattern as `AuthViewModel` and `NetworkMonitor`. |
