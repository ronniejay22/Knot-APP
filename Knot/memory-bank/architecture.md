...
| `OnboardingDislikesView.swift` | 4 | **Active (Step 3.4).** Dark-themed 3-column card grid matching InterestsView visual style. Displays all 40 interest categories; interests already selected as likes in Step 3.3 appear grayed out with heart badge and `.disabled(true)`. User selects exactly 5 dislikes. Contains `DislikeImageCard` (private struct) with 4 visual states: unselected (gradient), selected/disliked (pink border + checkmark), disabled/liked (flat gray, 50% opacity, heart badge), shaking (6th attempt rejection). Reuses `OnboardingInterestsView.cardGradient(for:)` and `.iconName(for:)` static methods for consistent visuals. Search bar, selection counter, and shake animation match the Interests screen. Calls `viewModel.validateCurrentStep()` via `.onAppear` and `.onChange(of: selectedDislikes)`. |
