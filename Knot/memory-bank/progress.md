...
### Step 19.18 ✅ Recommendations — Celebration Beat Before the Purchase Rating Sheet
**Date:** 2026-07-30
**Status:** Complete

**Goal:** After a merchant handoff, tapping "Yes, I bought it!" on the return-to-app prompt (`PurchasePromptSheet`, Step 9.4 / type-aware Step 19.16) jumped straight to the "How was this pick?" rating sheet (`PurchaseRatingSheet`) — the user had just done a meaningful thing for their partner and was immediately asked to grade it, with no moment of positive reinforcement. This inserts a short celebratory popover **between** the "Yes" tap and the rating step, reading "{Partner} is going to love it! Good choice!", that auto-advances into the rating after a brief beat.

**Approach:** Client-only, presentational — no backend change and no new `recommendation_feedback` action (the "Yes" tap still records `"purchased"` as before; the celebration records nothing). The new sheet mirrors `AppReviewPromptSheet` (the established `.medium` celebratory-sheet template): `Theme.backgroundGradient` container, `Theme.accent` heart icon, Fraunces (`Theme.Typography.cardTitle`) headline. Auto-advance is confirmed with the user (vs a tap-through button) and is split into a schedule/advance pair so the transition is unit-testable without waiting out the delay, and guarded so a manual swipe-away of the celebration cancels cleanly instead of popping the rating sheet on top.

**What changed (iOS):**
- **`iOS/Knot/Features/Recommendations/PurchaseCelebrationSheet.swift` (new):** display-only `.medium` bottom sheet taking `partnerName: String?`. Headline interpolates the partner name ("Alex is going to love it!") and falls back to the neutral "They're going to love it!" when the name is nil/empty (matching the app's "your partner" fallback convention); "Good choice!" subtitle underneath. No buttons — it auto-advances.
- **`iOS/Knot/Features/Recommendations/RecommendationsViewModel.swift`:** added `showPurchaseCelebration` state and a `celebrationAdvanceDelay` (2s). `confirmPurchase()` now dismisses the purchase prompt and shows the celebration (the `.success` haptic now lands here) instead of setting `showRatingPrompt` directly, then calls `scheduleCelebrationAdvance()`. New `scheduleCelebrationAdvance()` (private, sleeps then advances) and `advanceFromCelebrationToRating()` (guards on `showPurchaseCelebration`, then swaps celebration→rating) drive the timed hand-off.
- **`iOS/Knot/Features/Recommendations/RecommendationsView.swift` and `iOS/Knot/Features/Onboarding/Steps/OnboardingCompletionView.swift`:** both present the new `.sheet(isPresented: $viewModel.showPurchaseCelebration)` before their existing rating sheet. Both were required — they run separate `RecommendationsViewModel()` instances but share `confirmPurchase()`, so without the onboarding host also presenting it the onboarding rating flow would have stalled on the now-unbound celebration flag.
- **Screenshot seam (`iOS/Knot/App/UITestScreenshotHarness.swift` + `iOS/KnotUITests/PRScreenshotTests.swift`):** added a `purchaseCelebration` harness key that renders `PurchaseCelebrationSheet(partnerName: "Alex")` standalone (mirroring the `purchasePromptDate` harness), and pointed the PR screenshot test at it (waiting on the "Good choice!" copy).

**Files created:**
- `iOS/Knot/Features/Recommendations/PurchaseCelebrationSheet.swift` — the celebration bottom sheet
- `docs/pr-screenshots/worktree-feat-purchase-celebration-popover.png` — captured PR screenshot

**Files modified:**
- `iOS/Knot/Features/Recommendations/RecommendationsViewModel.swift` — celebration state + timed auto-advance
- `iOS/Knot/Features/Recommendations/RecommendationsView.swift` — present the celebration sheet
- `iOS/Knot/Features/Onboarding/Steps/OnboardingCompletionView.swift` — present the celebration sheet (shared VM path)
- `iOS/Knot/App/UITestScreenshotHarness.swift` — `purchaseCelebration` key + harness view
- `iOS/KnotUITests/PRScreenshotTests.swift` — point the screenshot slot at the new key
- `iOS/KnotTests/MerchantHandoffTests.swift` — updated/added celebration-flow tests
- `iOS/Knot.xcodeproj/project.pbxproj` — regenerated by XcodeGen to include the new file

**Tests:** iOS Full plan green (exit 0). Updated `ReturnToAppTests.testConfirmPurchaseShowsCelebration` (was `…ShowsRating`) to assert `confirmPurchase()` shows the celebration and *not* the rating; added `testAdvanceFromCelebrationShowsRating` and `testAdvanceFromCelebrationNoOpWhenDismissed` (guard) and `PurchasePromptViewTests.testPurchaseCelebrationSheetRenders` (named + nil-name fallback render). No backend layer touched.

**Notes:** `testLaunchPerformance` (a pre-existing UI app-launch performance test) intermittently fails the whole `-testPlan Full` run with exit 65 when the simulator is under load — its relative-standard-deviation gate (10%) trips at ~24% variance during back-to-back runs — while every functional test still passes; a calm re-run measures ~4% variance and exits 0. It is unrelated to this change (nothing here touches the launch path).
