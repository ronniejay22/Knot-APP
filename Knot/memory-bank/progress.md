...
### Step 18.12: Bundle Fraunces (Variable Font) and DM Sans Static Cuts ✅
**Date:** May 17, 2026
**Status:** Complete

**What was done:**

Step 18.10 introduced the `Theme.Typography` tokens that reference Fraunces and DM Sans via `Font.custom`, but the actual `.ttf` files were not yet bundled — the app was silently falling back to San Francisco for every Fraunces/DM Sans call site because `Font.custom` returns the system font when its PostScript name isn't registered. This step finishes the migration by bundling the font binaries, picking the variable-font approach for Fraunces, and adjusting the token implementations to match what's actually shipped.

Bundled five `.ttf` files at `iOS/Knot/Resources/Fonts/`: the Fraunces variable font (`Fraunces-VariableFont_SOFT,WONK,opsz,wght.ttf` — family `Fraunces`) and the four DM Sans static cuts (`DMSans-Regular.ttf`, `DMSans-Medium.ttf`, `DMSans-SemiBold.ttf`, `DMSans-Bold.ttf`). Updated `Info.plist` `UIAppFonts` to list those five filenames. Ran `xcodegen generate` so `Knot.xcodeproj/project.pbxproj` picked up each file in the Knot target's Copy Bundle Resources phase.

Refactored `Theme.Typography` to match the bundled assets. `FontFamily.fraunces` collapsed from three static-cut PostScript names to a single family name `"Fraunces"`. Each Fraunces token now picks its weight axis via `.weight(.light)` against the variable font: `heroDisplay` and `sectionHeader` request `.light` (300), `cardTitle` takes the default (400). The `italicQuote` token uses `.weight(.light).italic()` — a synthesized italic, since only the upright variable font is bundled and not the separate italic variable font. DM Sans tokens were not touched; the four static cuts already matched the previously declared PostScript names. Updated `Theme.registerFonts()` to expect family name `"Fraunces"` (it was already correct for `"DM Sans"`).

The original Step 18.10 plan called for "weight is baked into each token, never chain weight" because chaining can re-substitute the system font when the chosen weight isn't in the active family — but that rule was written for static cuts. Variable fonts handle weight via a continuous axis, and SwiftUI's `Font.custom(...).weight(...)` does the right thing on a variable font with a `wght` axis. The chaining now lives inside the Theme tokens (one place, not 270 call sites), so view code still treats each `Theme.Typography.*` as an opaque `Font` value with no further chaining required.

**Files modified:**
- `iOS/Knot/Info.plist` — replaced three Fraunces filename entries with the single variable-font filename
- `iOS/Knot/Core/Theme.swift` — `FontFamily.fraunces` collapsed to the family name; Fraunces tokens use `.weight(...)` for the variable font's weight axis; `italicQuote` uses `.italic()` synthesis with a docs note about the upgrade path to a true italic cut; `registerFonts()` expected-family list updated
- `iOS/Knot.xcodeproj/project.pbxproj` — regenerated by `xcodegen generate` to add `PBXBuildFile` + `PBXFileReference` entries for the five new `.ttf` files in the Copy Bundle Resources phase

**Files added:**
- `iOS/Knot/Resources/Fonts/Fraunces-VariableFont_SOFT,WONK,opsz,wght.ttf` (357 KB)
- `iOS/Knot/Resources/Fonts/DMSans-Regular.ttf` (55 KB)
- `iOS/Knot/Resources/Fonts/DMSans-Medium.ttf` (55 KB)
- `iOS/Knot/Resources/Fonts/DMSans-SemiBold.ttf` (55 KB)
- `iOS/Knot/Resources/Fonts/DMSans-Bold.ttf` (55 KB)

**Test results:**
- ✅ `xcodebuild build` succeeds with zero errors and zero warnings
- ✅ Manual verification on iPhone 17 Pro simulator: Fraunces renders on `Onboarding > Important Dates` heading; DM Sans renders on body copy and form labels. The `🔤` debug print from `Theme.registerFonts()` reports `✅ Fraunces loaded` and `✅ DM Sans loaded` with the bundled face names

**Notes:**
- Fraunces variable font axes: `opsz` (optical size), `wght` (weight), `SOFT` (softness), `WONK` (wonky letterform alternates). SwiftUI's `.weight(...)` only addresses the `wght` axis. To address `opsz` per-token (e.g., snap a 28pt heading to the 24pt opsz instance for cleaner letterforms), a follow-up could build a `UIFontDescriptor` with explicit variations and wrap it in `Font`, but that's not needed for shipping.
- The italic Fraunces variable font (`Fraunces-Italic-VariableFont_*.ttf`) is downloaded but not bundled. To replace the synthesized italic with the true italic cut, drop the file into `Resources/Fonts/`, add it to `Info.plist UIAppFonts`, add a `FontFamily.fraunces` italic constant pointing at its family name (likely `"Fraunces"` since the italic VF shares the family) or PostScript name, run `xcodegen generate`, and swap the `italicQuote` definition from `.weight(.light).italic()` to `.custom(italicName, size: 17, relativeTo: .body).weight(.light)`.
- The 72pt static cuts that briefly lived in `Resources/Fonts/` (`Fraunces_72pt-Light.ttf`, etc.) were removed in this step — they were never bundled in a tracked commit so the deletion is a no-op for git. The variable font is a single-file replacement for that whole set.
- Both Fraunces and DM Sans ship under SIL OFL 1.1, which permits commercial embedding without in-app attribution.
...
### Step 19.4 ✅ Recommendations — Guarantee Every Card Is Bookable-or-Idea (Never a Web-Search Link)
**Date:** 2026-07-01
**Status:** Complete
...
- **iOS (`Models/DTOs.swift`, `Features/Recommendations/RecommendationDetailView.swift`):** Removed `externalUrlIsSearch`/"Find it online"; `primaryCTA` shows "Open in <merchant>" only when a real link exists (`hasOpenableLink`), otherwise degrades to the Save action. The screenshot is captured via a new `"recDetail"` key on the shared `UITestScreenshotHarness` (the seam added in Step 19.3), rendering `RecommendationDetailView` with a bookable sample; `PRScreenshotTests` also dismisses transient simulator system alerts.

**Tests:** Backend `1290 passed` — `test_url_resolution.py` now covers domain rejection (incl. precise search-engine matching) + scoring + resolve-leaves-None; `test_availability_node.py` covers resolve-backup-before-swap, bookable-preferred-over-idea, last-resort-idea, and linkless-idea fallbacks (off the old search-URL asserts); `test_pipeline.py` fixture updated. iOS `RecommendationDTOTests` green (null-external_url decode). PR screenshot shows the bookable "Open in <merchant>" CTA.
...
### Step 19.22 ✅ Notifications — Milestone Push Tap-Through to Pre-Generated Recommendations
**Date:** 2026-08-01
**Status:** Complete

**Goal:** Close the last gap in the milestone push-notification loop. The Phase 7 backend chain already worked end-to-end — milestone create/update schedules 14/7/3-day notifications, the QStash webhook pre-generates 3 recommendations linked to the milestone, stores a Claude briefing, and delivers an APNs push whose body is the briefing snippet — but **tapping the push was a dead end**: `AppDelegate.userNotificationCenter(_:didReceive:)` extracted `milestone_id`/`notification_id`, `print()`ed them, and stopped at a literal Step 9.2 TODO. The pre-generated recommendations were also unreachable from the main UI (their only reader, `NotificationsView`, is dead code) and were served degraded (no `personalization_note`, no briefing). Now: tap the push → the app opens directly on the pre-generated recommendations for that milestone — instantly, with no ~30s pipeline re-run — showing exactly what the push described. Push copy also moved to the requested friendlier tone.

**What changed (backend):**
- **`app/api/notifications.py`:** the webhook's `recommendations` insert now mirrors `POST /generate`'s row construction — guaranteed `image_url` via `resolve_image_url` (imported from `app.api.recommendations`; no circular import), `personalization_note`, and idea fields (`is_idea`, JSON-encoded `content_sections`). Previously the insert silently dropped the note (the column has existed since migration 00021), so webhook-stored rows read back with no "Why Knot picked this."
- **`app/api/recommendations.py` (`GET /by-milestone/{milestone_id}`):** returns the **newest batch of 3** (was newest 10 — each generation run inserts its trio in one call, so `order created_at desc, limit 3` is exactly the latest Choice-of-Three; a later 7-day fire or manual For You generate naturally supersedes a stale 14-day batch). Maps `personalization_note` (through `trim_to_complete_sentence`), `is_idea`, and JSON-decoded `content_sections` per item, and best-effort joins the newest `milestone_briefings.briefing_text` into the response — the briefings table's first read path (it was write-only since Step 17.1). Briefing lookup failure is non-fatal (`briefing_text: null`).
- **`app/models/notifications.py`:** `MilestoneRecommendationItem` += `personalization_note`/`is_idea`/`content_sections`; `MilestoneRecommendationsResponse` += `briefing_text`. All defaulted → legacy rows and the dead history view stay compatible.
- **`app/services/apns.py`:** friendlier copy. Title now uses per-cadence phrasing via `_DAYS_PHRASE` — "Jas's Birthday **is two weeks away** / **is next week** / **is in 3 days**" (fallback "is in N days") — and the no-briefing fallback body is `FALLBACK_BODY`: *"Have you gotten them anything yet? Tap for a few ideas we picked out."* (replacing "I've found N {Vibe} options…"). The briefing snippet remains the primary body. `build_notification_payload` dropped its now-unused `vibes`/`recommendations_count` params; `deliver_push_notification`'s signature is unchanged for caller stability.

**What changed (iOS):**
- **`Core/DeepLinkHandler.swift`:** added `case milestoneRecommendations(milestoneId:notificationId:)`, a `static let shared` singleton (the `DeviceTokenService.shared` precedent — AppDelegate fires independently of the SwiftUI hierarchy; `KnotApp` now injects this same instance so delegate writes and view observation agree), and the pure, unit-tested `static destination(milestoneId:notificationId:)` payload→destination mapper (nil/empty milestone id → nil; empty notification id normalized to nil).
- **`App/AppDelegate.swift`:** the tap handler's TODO is gone — it extracts the String values inside the `@MainActor` delegate (the non-Sendable `userInfo` never crosses an isolation boundary) and sets `DeepLinkHandler.shared.pendingDestination`.
- **`App/ContentView.swift`:** deep-link consumption extracted into `consumePendingDeepLink()` handling both destination cases; the milestone case presents `MilestoneRecommendationsCoverView` via `.fullScreenCover(item:)` with an explicit `.environment(authViewModel)` (cover content attached outside the Group's environment modifier doesn't inherit it). **Latent-bug fix:** the `.task` now consumes the pending deep link BEFORE `await authViewModel.listenForAuthChanges()` — that call is a never-returning `for await`, so the previous cold-start check placed after it was dead code. This also repairs the Step 9.2 `.recommendation` cold-start path. Warm-start `.onChange` and cold-start `.task` cover both didReceive-vs-task orderings.
- **`Features/Recommendations/MilestoneRecommendationsCoverView.swift` (new):** NavigationStack shell with a close (X) button; best-effort milestone lookup via `MilestoneService.listMilestones()` to build the `MilestoneDisplayContext` (the push only carries the milestone's ID; proceeds with nil context on failure); fire-and-forget `NotificationHistoryService.markViewed`; hosts `RecommendationsView(preferPregenerated: true, isModal: true)`.
- **`Features/Recommendations/RecommendationsViewModel.swift`:** new 1-method `MilestoneRecommendationsFetching` protocol (conformed by the existing `NotificationHistoryService` — no duplicated URLSession boilerplate; injected via init for testability) and `loadPregeneratedRecommendations(milestoneId:) async -> Bool`: success → map rows via `toRecommendationItem()`, set `briefingText`/`hasLoadedInitially`, bump `deckResetToken`, return `true`; empty → return `false` (caller runs the generate fallback); error → set `errorMessage` and return `true` so Try Again re-runs the cheap fetch instead of silently burning a pipeline run.
- **`Features/Recommendations/RecommendationsView.swift`:** `preferPregenerated`/`isModal` params + an explicit init with a `viewModel:` seam (defaults preserve the ForYouView call sites); `.task` guards on `hasLoadedInitially` and routes through a single `loadContent()` (preloaded-first, generate fallback), which the error/empty retry buttons also use; `generateWithMilestoneContext()` no longer drops the milestoneId when the context lookup failed; carousel bottom padding `isModal ? 24 : 100` (no KnotTabBar inside a cover).
- **`Models/DTOs.swift`:** `MilestoneRecommendationsResponse` += `briefingText`; `MilestoneRecommendationItemResponse` += `personalizationNote`/`isIdea`/`contentSections` (all optional — legacy JSON still decodes) + `toRecommendationItem()` mapping into the full detail-capable item (source `"milestone_pregenerated"`) via the Step 19.9 in-code init, inheriting tag-token sanitation.
- **Screenshot seam:** new `milestoneRecs` harness key renders the tap-through destination with a seeded VM (3 `PreviewRecommendations` items + briefing, `hasLoadedInitially = true` → zero networking); `PRScreenshotTests` waits on the "Jas's Birthday" title + "Knot's Take" briefing header.

**Files created:**
- `iOS/Knot/Features/Recommendations/MilestoneRecommendationsCoverView.swift` — full-screen tap-through shell
- `iOS/KnotTests/MilestonePushTapThroughTests.swift` — 14 tests: `DeepLinkHandlerTests` (destination mapping + `handleURL` regression), `MilestoneRecommendationDTOTests` (enriched + legacy decode, `toRecommendationItem` field/idea mapping + sanitation), `PregeneratedRecommendationsViewModelTests` (mock-fetcher success/empty/error/guard paths)

**Files modified:**
- `backend/app/api/notifications.py`, `backend/app/api/recommendations.py`, `backend/app/models/notifications.py`, `backend/app/services/apns.py` — per above
- `backend/tests/test_apns_push_service.py` — `TestBuildNotificationPayload` rewritten for the new copy (per-cadence titles, friendly fallback, briefing-primary, empty-snippet fallback); `test_builds_correct_payload` updated
- `backend/tests/test_notification_processing.py` — new `test_processing_persists_personalization_note` (captures the webhook insert; asserts note + idea fields + non-null images)
- `backend/tests/test_notification_history.py` — by-milestone tests enriched (note/idea round-trip, JSON-string `content_sections` decoded, `limit(3)` asserted, briefing joined, briefing-failure non-fatal, model defaults)
- `backend/tests/test_milestone_crud.py` — `TestApnsBriefingSnippet` updated to the new `build_notification_payload` contract
- `iOS/Knot/App/AppDelegate.swift`, `ContentView.swift`, `KnotApp.swift`, `UITestScreenshotHarness.swift`, `iOS/Knot/Core/DeepLinkHandler.swift`, `iOS/Knot/Features/Recommendations/RecommendationsView.swift`, `RecommendationsViewModel.swift`, `iOS/Knot/Models/DTOs.swift`, `iOS/KnotUITests/PRScreenshotTests.swift` — per above

**Tests:** Full backend suite: 1349 passed, 622 skipped, 0 failures (offline default; live tests run with `--integration`). iOS Full plan (unit + UI) green, including the 14 new `MilestonePushTapThroughTests`. PR screenshot captured via the `milestoneRecs` harness showing the milestone header, briefing card, and carousel.

**Notes:**
- Manual end-to-end tap verification without QStash/APNs: boot the backend (`/start-server`), then `xcrun simctl push booted com.ronniejay.knot payload.apns` with `{"Simulator Target Bundle": "com.ronniejay.knot", "aps": {"alert": {...}}, "milestone_id": "<real milestone uuid>", "notification_id": "<queue row uuid or omit>"}` — tapping the banner exercises AppDelegate → DeepLinkHandler → ContentView cover with the real fetch.
- Stale-data behavior: `limit(3)` newest-batch semantics means a tap on an old 14-day push shows the freshest trio for that milestone (possibly from a later fire or a manual generate) — better than replaying stale rows. A deleted milestone → empty fetch → generate fallback errors → the standard error state with Try Again.
- Out of scope (noted follow-ups): Snooze/View action buttons (`MILESTONE_REMINDER` category registration), resurrecting the NotificationsView history surface, widening the 14/7/3 cadence, and the webhook marking notifications `sent` when generation produced 0 recs.
...
**Goal:** Collapse the two-family typographic system (Fraunces serif for display/headings, DM Sans sans for body/UI — Steps 18.10 / 18.12) down to DM Sans alone, with every token keeping its **exact** current size and weight.

**Why it was a small change:** all Fraunces usage funnels through `Theme.Typography`, and the 62 view-code call sites name tokens rather than families (`.knotFont(Theme.Typography.cardTitle)`). So no view needed retyping — only the token definitions, plus the one place that had bypassed the design system.

**The one thing that had to be sourced: a Light cut.** Three tokens are weight 300, and DM Sans Light was not among the four bundled static cuts. `google/fonts` now publishes **only** the DM Sans variable font — the four cuts in the repo came from a legacy Google Fonts static download that no longer exists. Rather than round Light up to Regular (which would have changed the weight the request asked to preserve), `DMSans-Light.ttf` was generated from the official OFL variable font:

```
fontTools.varLib.instancer DMSans[opsz,wght].ttf wght=300 opsz=14 --update-name-table
```

`wght=300, opsz=14` are the coordinates of the font's own named **"Light"** instance, so the optical size matches the four cuts already bundled. The instancer names an `opsz` instance `"DM Sans 14pt Light"` / PostScript `"DMSans-14ptLight"`, which would **not** have resolved — so the `name` table was patched to the Google static convention the existing cuts use (nameID 1 `DM Sans Light`, 2 `Regular`, 4 `DM Sans Light`, 6 `DMSans-Light`, 16 `DM Sans`, 17 `Light`). iOS groups by the typographic family (16), which is what puts all five faces under one `DM Sans` family.

**Token mapping — size and weight unchanged in every row:**

| Token | Was | Now |
|---|---|---|
| `heroDisplay` | Fraunces Light 42 / `.largeTitle` | DMSans-Light 42 / `.largeTitle` |
| `sectionHeader` | Fraunces Light 28 / `.title` | DMSans-Light 28 / `.title` |
| `sectionHeaderSemibold` | Fraunces SemiBold 28 / `.title` | DMSans-SemiBold 28 / `.title` |
| `onboardingHeader` | Fraunces SemiBold 32 / `.title` | DMSans-SemiBold 32 / `.title` |
| `onboardingSubHeader` | Fraunces SemiBold 20 / `.title2` | DMSans-SemiBold 20 / `.title2` |
| `cardTitle` | Fraunces Regular 20 / `.title2` | DMSans-Regular 20 / `.title2` |
| `italicQuote` | Fraunces Light 17 + synthesized `.italic()` | DMSans-Light 17 + synthesized `.italic()` |

**Every `.weight(...)` chain was removed.** Fraunces is a variable font, so its tokens selected weight by chaining `.weight(.light)` / `.weight(.semibold)` against the `wght` axis. DM Sans ships static cuts, where weight belongs to the PostScript name — and `Theme.swift`'s own docs already warned that chaining a weight the active family doesn't carry makes iOS silently substitute San Francisco. Leaving those chains in would have been the exact failure the file warns about.

**What changed:**
- **`iOS/Knot/Core/Theme.swift`:** `FontFamily.fraunces` deleted, `FontFamily.DMSans.light` added; the seven tokens above repointed per the table with their `.weight(...)` chains dropped; doc comments rewritten (the enum-level "two-family pairing" note, each token's family/weight line, and `italicQuote`'s synthesized-italic upgrade path, now `DMSans-LightItalic.ttf`). `modalTitle` lost its "deliberate exception to the app's Fraunces-for-headings convention" note — there is no such convention left to except. `registerFonts()` expects `["DM Sans"]`.
- **`iOS/Knot/Features/Onboarding/Steps/Shared/OnboardingStepHeader.swift`:** the last `Font.custom("Fraunces", …)` outside `Theme.swift` — a 28pt SemiBold override — now reads the new `Theme.Typography.onboardingHeaderCompact` token, so no font family name is spelled outside the design system.
- **New token `onboardingHeaderCompact`** (DMSans-SemiBold 28, `relativeTo: .title`). Post-swap it is value-identical to `sectionHeaderSemibold`, and the first draft simply aliased that — but code review caught that this couples ~15 onboarding step headers to a token whose only other consumer is the recommendation detail title, so a future tweak to either would silently retype the other. A dedicated token follows the precedent set by `onboardingHeader` vs `sectionHeader` (Steps 18.22 / 18.23), where the same reasoning produced the same answer.
- **`iOS/Knot/Resources/Fonts/`:** `Fraunces-VariableFont_SOFT,WONK,opsz,wght.ttf` (357 KB) deleted, `DMSans-Light.ttf` (57 KB) added — a net ~300 KB smaller bundle.
- **`iOS/Knot/Info.plist`:** the Fraunces `UIAppFonts` entry replaced with `DMSans-Light.ttf`.
- **`iOS/Knot.xcodeproj/project.pbxproj`:** regenerated by `xcodegen generate` (fonts are auto-included by the target's `sources: - path: Knot` rule, so `project.yml` needed no edit).
- **`iOS/Knot/Features/Auth/SignInView.swift`:** the "signature italic Fraunces brand moment" comment on the tagline reworded.

**Files created:**
- `iOS/Knot/Resources/Fonts/DMSans-Light.ttf`

**Tests:** `ThemeTokensTests` gained two real registration guards. The existing typography tests are existence-only compile checks that would have passed even if every face silently fell back to San Francisco — the exact failure `Theme.registerFonts()` prints about but that nothing failed CI on. `testDMSansIsTheOnlyBundledFontFamily` asserts `UIFont.familyNames` contains `"DM Sans"` and **not** `"Fraunces"`; `testEveryDMSansCutIsRegistered` asserts all five PostScript names resolve under that family. Confirmed at runtime — `registerFonts()` now prints `✅ DM Sans loaded — faces: ["DMSans-Regular", "DMSans-Light", "DMSans-Medium", "DMSans-SemiBold", "DMSans-Bold"]` with no Fraunces line. Full plan (unit + UI) green.

**Screenshot:** the PR image captures **login**, via a new `login` harness key — its "Create Account" title is `sectionHeader`, the most-used of the three tokens backed by the hand-instanced `DMSans-Light.ttf`. Getting there took three attempts, each for a different reason worth recording:

1. **`recDetail`** — chosen because it stacks the widest spread of retyped tokens in one frame. Code review pointed out it renders *none* of `heroDisplay`, `sectionHeader` or `italicQuote`, i.e. none of the Light tokens. The Light cut is the only face in this change not shipped by Google, so it is precisely the one needing proof it **renders** rather than merely registers; the other four already shipped on main.
2. **`signIn`** — carries two Light tokens (the 42pt wordmark and the italic tagline), so it looked ideal, and the captured image was correct. But `SignInView` runs `PhotoGridSection`: 40 photo tiles redrawn continuously by a `TimelineView` (Step 18.20 documented it pegging a core before that step's fixes). Under a UI test that kept the app busy enough for the runner to kill it at teardown — **every** Full-plan run on this target lost two UI tests to `signal kill` / `signal term`, and never the same two. A control run of the UI bundle against unmodified `main` passed, and the branch's own pre-`signIn` Full run had been 431/431, which is what identified the target rather than the font change as the cause.
3. **`login`** — a static screen showing `sectionHeader` (DMSans-Light 28). Stable, and still proof the instanced cut renders.

A harness key was needed at all because both screens sit on the unauthenticated path: a simulator holding a Supabase session routes straight past them. That bit first — an attempt without a key landed on For You behind a notification permission alert and still reported success, because every wait in `PRScreenshotTests` discarded its result (`_ = …waitForExistence`). The title wait is now an `XCTAssertTrue`, so a screenshot run that lands on the wrong screen fails loudly instead of shipping a useless image.
...
85. **Greedy diversity selection algorithm (Step 5.6):** The `select_diverse_three` node uses a greedy approach: pick the highest-scored candidate first, then for each subsequent pick, select the candidate that maximizes a 0–3 diversity score across three dimensions (price tier, type, merchant). Ties in diversity are broken by `final_score` (higher wins), then alphabetical title. This is O(n×k) where n=pool size and k=3 picks — fast enough for 9 candidates.

86. **Price tier classification (Step 5.6):** `_classify_price_tier()` splits the budget range into three equal bands (low/mid/high). Candidates with `None` price default to "mid" (neutral). A zero budget range (min == max) also defaults to "mid". This prevents division-by-zero and keeps None-priced candidates from biasing toward low or high.
...
110. **Database migration 00013 adds device_token columns to users table (Step 7.4):** `ALTER TABLE public.users ADD COLUMN IF NOT EXISTS device_token TEXT, ADD COLUMN IF NOT EXISTS device_platform TEXT CHECK (device_platform IN ('ios', 'android'))`. A partial index `idx_users_device_token ON public.users (device_token) WHERE device_token IS NOT NULL` optimizes push delivery lookups in Step 7.5. Columns are nullable — NULL means the user hasn't granted notification permissions or hasn't launched the app since this migration. One device per user for MVP; a separate `device_tokens` table can replace this if multi-device support is needed later.

111. **DeviceTokenRequest uses @field_validator for platform validation (Step 7.4):** The Pydantic model in `backend/app/models/users.py` validates `device_token` (stripped, non-empty, max 200 chars) and `platform` (must be "ios" or "android", defaults to "ios"). The validator pattern mirrors `models/hints.py` — `@field_validator("device_token", mode="before")` strips whitespace before length checks, preventing whitespace-only tokens from passing validation.

112. **POST /api/v1/users/device-token returns "registered" vs "updated" status (Step 7.4):** The endpoint in `backend/app/api/users.py` checks the existing `device_token` column before updating. If the column was NULL (first registration), the response `status` is `"registered"`; if it had a previous value, the status is `"updated"`. This distinction enables the iOS app (or monitoring) to distinguish first-time registrations from token refreshes. The endpoint uses `get_service_client()` to bypass RLS since it writes to the `users` table directly (not vault-scoped).

113. **AppDelegate bridges UIKit push callbacks into SwiftUI lifecycle (Step 7.4):** `iOS/Knot/App/AppDelegate.swift` is a `@MainActor final class` conforming to `UIApplicationDelegate` and `@preconcurrency UNUserNotificationCenterDelegate`. SwiftUI has no native hooks for `didRegisterForRemoteNotificationsWithDeviceToken` or `didFailToRegisterForRemoteNotificationsWithError`, so a UIKit delegate is required. The `@preconcurrency` annotation on `UNUserNotificationCenterDelegate` resolves Swift 6 strict concurrency errors where `UNUserNotificationCenter` and `UNNotification` are non-Sendable types being passed into a `@MainActor`-isolated method. Without `@preconcurrency`, the `willPresent` delegate method produces two compile errors under `SWIFT_STRICT_CONCURRENCY: complete`.

114. **DeviceTokenService uses singleton pattern for AppDelegate access (Step 7.4):** `iOS/Knot/Services/DeviceTokenService.swift` uses `static let shared` because the AppDelegate fires `didRegisterForRemoteNotificationsWithDeviceToken` before the SwiftUI view hierarchy exists — environment injection is not available. The service is `@MainActor final class: Sendable` matching `HintService`'s patterns: `getAccessToken()` from Supabase session, `mapURLError()` for typed network errors, `parseErrorMessage()` for backend error responses. Registration is fire-and-forget — failures are logged but never surfaced to the user.

115. **Push notification permission requested after vault check, not before (Step 7.4):** In `AuthViewModel.swift`, `requestPushNotificationPermission()` is called after the vault existence check completes in both `.initialSession` and `.signedIn` cases. This ensures the permission dialog appears only after the user is authenticated and the app knows whether to show Home or Onboarding. If called earlier, the permission dialog could overlap with the loading state or appear before the user sees the main UI. The method uses `Task { @MainActor in ... }` to ensure `UIApplication.shared.registerForRemoteNotifications()` runs on the main thread.

116. **`aps-environment` entitlement added to both entitlements file and project.yml (Step 7.4):** The `aps-environment: development` key was added to `iOS/Knot/Knot.entitlements` (the actual entitlements plist) and to `iOS/project.yml` under `entitlements.properties` (so XcodeGen includes it when regenerating the project). Both must be in sync — the entitlements file is what Xcode signs with, and project.yml is what XcodeGen uses to generate the Xcode project configuration.

117. **Device token hex encoding uses `map { String(format: "%02x", $0) }.joined()` (Step 7.4):** The raw `Data` from `didRegisterForRemoteNotificationsWithDeviceToken` is converted to a lowercase hex string. Each byte becomes exactly 2 hex characters (`%02x` format specifier), producing a 64-character string for a typical 32-byte APNs device token. This matches the format expected by most APNs libraries (including the Python `aioapns` planned for Step 7.5).

118. **Backend device token tests: 20 tests across 6 classes (Step 7.4):** `backend/tests/test_device_token_api.py` covers: TestValidRegistration (3 — register returns 200, default platform "ios", explicit "android" platform), TestTokenUpdate (1 — second registration returns "updated" status), TestDatabaseStorage (3 — token stored in DB, second token replaces first, initial token is NULL), TestValidationErrors (5 — empty token 422, whitespace-only 422, invalid platform 422, missing field 422, token too long 422), TestAuthRequired (2 — no header 401, invalid token 401), TestModuleImports (6 — models importable, router importable, router registered in app, request validation, default platform, response model). Test helpers follow the `test_hint_submission_api.py` pattern: `_supabase_configured()` skip guard, `_admin_headers()`, `_create_auth_user()`, `_sign_in_user()`, `_query_user_device_token()` for direct DB verification.

119. **APNs push notification service created in `backend/app/services/apns.py` (Step 7.5):** The service handles the complete APNs push delivery lifecycle: loading the `.p8` ES256 private key from disk (`_load_auth_key()`), generating JWT tokens with 50-minute caching (`_generate_apns_token()` using PyJWT with `algorithm="ES256"` and `kid` header), building structured notification payloads (`build_notification_payload()`), and sending via HTTP/2 to APNs (`send_push_notification()` using `httpx.AsyncClient(http2=True)`). The high-level entry point `deliver_push_notification()` looks up the user's `device_token` from the `users` table, builds the payload with vault/milestone data, and sends. All failures return structured result dicts (`success`, `apns_id`, `status_code`, `reason`) — no exceptions escape.

120. **APNs notification payload format follows Apple's specification (Step 7.5):** The payload structure uses `aps.alert.title` and `aps.alert.body` (not the shorthand string form) for rich notification display. Title format: `"{partner_name}'s {milestone_name} is in {days_before} days"`. Body format: `"I've found {N} {Vibe} options based on their interests. Tap to see them."` where the first vibe tag is capitalized with underscores replaced by spaces. Category `"MILESTONE_REMINDER"` enables custom notification actions (View/Snooze) registered in the iOS app. Custom data keys `notification_id` and `milestone_id` sit at the top level of the payload for deep-linking. Sound is `"default"`.

121. **APNs JWT token uses ES256 algorithm with `kid` header (Step 7.5):** Apple requires ES256 (ECDSA with P-256 curve) for provider authentication tokens. The JWT header includes `kid` (Key ID from Apple Developer) and `alg: ES256`. Claims include `iss` (Team ID) and `iat` (issued-at timestamp). Tokens are cached for 50 minutes (Apple allows up to 60 minutes before `ExpiredProviderToken` errors). The `cryptography` package was added to `requirements.txt` as a PyJWT backend for ES256 signing — the default PyJWT install only supports HMAC algorithms.

122. **APNs push delivery integrated into notification webhook with graceful degradation (Step 7.5):** The push delivery section (Step 7 in the webhook's processing pipeline) sits between recommendation generation and status update. It only fires when `is_apns_configured()` returns `True` AND `recommendations_count > 0` — no push is sent if recommendations failed or APNs credentials are missing. The entire block is wrapped in `try/except Exception` matching the graceful degradation pattern established in Step 7.3. Push failure (APNs rejection, connection timeout, missing device token) never prevents the notification from being marked as 'sent'. The `push_delivered: bool` field was added to `NotificationProcessResponse`.

123. **Config follows existing QStash pattern for APNs variables (Step 7.5):** Five new environment variables added to `config.py`: `APNS_KEY_ID`, `APNS_TEAM_ID`, `APNS_AUTH_KEY_PATH` (filesystem path to .p8 key), `APNS_BUNDLE_ID` (app's bundle identifier for the `apns-topic` header), and `APNS_USE_SANDBOX` (bool, defaults to `true` for development). Two validation functions match the QStash pattern: `validate_apns_config()` and `is_apns_configured()` — both return `bool`, both check all four credential vars. `.env.example` updated with `APNS_BUNDLE_ID=` and `APNS_USE_SANDBOX=true`.

124. **`deliver_push_notification()` handles missing device tokens without raising (Step 7.5):** When the user's `device_token` column is NULL (they haven't granted notification permission) or the user doesn't exist in the `users` table, the function returns `{"success": False, "reason": "no_device_token"}` instead of raising an exception. This prevents push delivery failures from propagating up to the webhook handler. Database lookup errors also return structured failure dicts with `reason: "device_token_lookup_failed: {exc}"`.
...
127. **`learned_weights` field added to `RecommendationState` for pre-loaded personalization data (Step 10.3):** The `learned_weights: Optional[UserPreferencesWeights] = None` field on `RecommendationState` follows the same pre-loaded data pattern as `vault_data` — weights are loaded by pipeline callers before execution, not by nodes during execution. This keeps LangGraph nodes stateless and free of DB access. The `UserPreferencesWeights` model is imported directly (not behind `TYPE_CHECKING`) because Pydantic needs runtime type resolution for field validation. Default is `None`, ensuring backward compatibility for users without feedback history.

128. **`load_learned_weights()` in `vault_loader.py` with graceful degradation (Step 10.3):** The function queries `user_preferences_weights` for a single user's stored weights. It lives in `vault_loader.py` rather than `feedback_analysis.py` because it's a pipeline input loader — colocated with `load_vault_data()`, `load_milestone_context()`, and `find_budget_range()` that are already imported by all three pipeline callers. The entire function is wrapped in `try/except Exception` returning `None` on failure, matching the graceful degradation pattern used throughout the pipeline. Failures are logged as warnings but never block recommendation generation.

129. **Interest weight multipliers applied in filtering node scoring (Step 10.3):** `_score_candidate()` now accepts an optional `interest_weights: dict[str, float] | None` parameter. Each interest match score is multiplied by the learned weight for that interest category (default 1.0 if no weight exists). The metadata bonus (+0.5 for exact catalog match) is deliberately NOT scaled by learned weights — it's a fixed signal strength bonus that should remain constant regardless of user preferences. `filter_by_interests()` extracts weights from `state.learned_weights.interest_weights` before scoring.

130. **Vibe, love language, and type weights applied in matching node (Step 10.3):** Three weight dimensions are applied in `match_vibes_and_love_languages()`: (1) `_compute_vibe_boost()` scales each matching vibe's +30% contribution by `vibe_weights.get(vibe, 1.0)`, (2) `_compute_love_language_boost()` scales primary and secondary boosts by `ll_weights.get(language, 1.0)`, (3) `type_weights.get(candidate.type, 1.0)` is applied as a separate final multiplier in the composite formula: `final_score = base × (1 + vibe_boost) × (1 + ll_boost) × type_weight`. Type weight is multiplicative (not additive) because it represents an overall preference for recommendation types (gift vs experience vs date), not a boost to a specific signal.
