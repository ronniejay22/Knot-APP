...
final class SavedViewModelTests: XCTestCase {
    ...
    /// In-memory SwiftData context for exercising load behavior.
    ...
    private func insert(_ context: ModelContext, id: String, savedAt: Date) {
    ...
    /// Verify load returns every saved item, most recently saved first — the
    /// one list the Saved tab shows.
    func testLoadListsEverySavedItemNewestFirst() throws {
        let context = try makeContext()
        insert(context, id: "old", savedAt: Date(timeIntervalSince1970: 1000))
        insert(context, id: "new", savedAt: Date(timeIntervalSince1970: 3000))
        insert(context, id: "mid", savedAt: Date(timeIntervalSince1970: 2000))
        ...
        XCTAssertEqual(vm.savedRecommendations.map(\.recommendationId), ["new", "mid", "old"])
    }
}
