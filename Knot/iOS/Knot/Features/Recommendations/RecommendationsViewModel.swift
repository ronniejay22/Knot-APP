...
protocol MilestoneRecommendationsFetching {
    ...
    /// and — with `justBecause: true` — the resume path for the Journal's
    /// "Surprise them today" picks, which must never resume a milestone's batch.
...
final class RecommendationsViewModel {
    ...
    /// When the batch on screen was generated. `Date()` for a run this session
    /// made; the stored row's `created_at` for a batch that was resumed,
    /// pre-generated by a push, or recovered. `nil` until something has been
    /// published. With `isResumedBatch`, drives the resume banner's copy.
    var batchGeneratedAt: Date?

    /// Whether the batch on screen was *resumed* — read back from storage on a
    /// Journal re-entry (`resumeFreshBatch`) rather than generated, pushed, or
    /// recovered this visit. Only a resumed batch shows the "Picking up where
    /// you left off" banner: a batch the user just made, or just tapped a push
    /// for, needs no explanation. Every other publishing path clears it.
    var isResumedBatch = false
    ...
    private(set) var backgroundTaskID: UIBackgroundTaskIdentifier = .invalid
    ...
    func loadPregeneratedRecommendations(milestoneId: String) async -> Bool {
        ...
        do {
            ...
            briefingText = response.briefingText
            hasLoadedInitially = true
            batchGeneratedAt = Self.batchTimestamp(of: response)
            isResumedBatch = false
            deckResetToken += 1
            return true
        } catch {
    ...
    // MARK: - Resume a Stored Batch (Journal re-entry)

    /// How long a stored batch stays the thing this screen shows on re-entry
    /// before a visit generates a fresh one. Milestone pushes run on a
    /// 14/7/3-day cadence, so a week keeps a batch alive until the next fire
    /// rather than regenerating on every visit in between. One constant, no
    /// server-side TTL: expired batches are simply not resumed, and the table
    /// stays append-only history for the feedback loop.
    static let resumableBatchWindow: TimeInterval = 7 * 24 * 60 * 60

    /// Whether a stored batch can stand in for a fresh generation on this
    /// surface: it belongs to the same context (`milestoneId` matches, `nil`
    /// for just-because), it is non-empty, its newest row's timestamp parses,
    /// and it is no older than `resumableBatchWindow`.
    ///
    /// Sibling of `isRecoverableBatch` with the id-disjointness check dropped —
    /// resume *wants* the batch that was on screen last time; recovery must
    /// reject it. An unparseable timestamp fails closed, as there.
    static func isResumableBatch(
        _ response: MilestoneRecommendationsResponse,
        milestoneId: String?,
        now: Date = Date()
    ) -> Bool {
        guard response.milestoneId == milestoneId else { return false }
        guard let createdAt = batchTimestamp(of: response) else { return false }
        return now.timeIntervalSince(createdAt) <= resumableBatchWindow
    }

    /// Shows the most recent stored batch for this context instead of running
    /// the pipeline, when one exists and is fresh enough.
    ///
    /// Every generation run is persisted server-side before it responds, so a
    /// user who generated picks, backed out, and came back should see the same
    /// picks — not a second ~25s wait. The push tap-through already reads its
    /// stored batch this way (`loadPregeneratedRecommendations`); this is the
    /// Journal's equivalent for both of its entry points.
    ///
    /// Best-effort by design: a thrown fetch error is swallowed and reported as
    /// "nothing to resume", because the caller's next move is to generate and a
    /// failed read must not stand in the way of that. It therefore never writes
    /// `errorMessage`, unlike the push path, whose Try Again re-runs the read.
    ///
    /// - Returns: `true` when a fresh batch was published; `false` when the
    ///   caller should generate (no batch, stale batch, wrong context, or the
    ///   read failed).
    func resumeFreshBatch(milestoneId: String?) async -> Bool {
        ...
        defer {
            isLoading = false
            // Backgrounding the app during this read takes the same background
            // window and schedules the same 1s "Still working on it…" notice
            // that a ~25s generation does (`handleAppBackgroundedWhileLoading`
            // keys off `isLoading`). A read that finishes in a few hundred
            // milliseconds must hand both back, or the assertion leaks until
            // the next generate ends and the notice fires for a wait that
            // never happened. Both calls no-op when nothing was taken.
            releaseBackgroundExecutionWindow()
        ...
        do {
            if let milestoneId {
                response = try await milestoneFetcher.fetchMilestoneRecommendations(
                    milestoneId: milestoneId
                )
            } else {
                response = try await milestoneFetcher.fetchLatestRecommendations(justBecause: true)
            }
        } catch {
            return false
        }

        guard Self.isResumableBatch(response, milestoneId: milestoneId) else { return false }

        recommendations = response.recommendations.map { $0.toRecommendationItem() }
        // `/latest` never carries a briefing; don't blank one a milestone read
        // returned on a surface that shows it.
    ...
    /// The newest row's `created_at`, parsed — the batch's generation time.
    /// `nil` for an empty response or an unparseable timestamp.
    static func batchTimestamp(of response: MilestoneRecommendationsResponse) -> Date? {
        guard let newest = response.recommendations.first else { return nil }
        return parseTimestamp(newest.createdAt)
    }

    /// How long ago a batch was made, as it reads inside a sentence: "earlier
    /// today" / "yesterday" / "N days ago". Calendar-day difference, so a batch
    /// from 11pm reads as "yesterday" at 1am rather than "today". A future
    /// timestamp (clock skew) clamps to "earlier today".
    static func batchAgePhrase(
        generatedAt: Date,
        now: Date = Date(),
        calendar: Calendar = .current
    ) -> String {
        let from = calendar.startOfDay(for: generatedAt)
    ...
    /// The resume banner's body: "These are the picks we found for Jas 2 days
    /// ago. Want a fresh set?" — the partner's name is dropped (not replaced)
    /// when it is unknown or blank.
    static func resumeBannerMessage(
        partnerName: String?,
        generatedAt: Date,
        now: Date = Date(),
        calendar: Calendar = .current
    ) -> String {
        let age = batchAgePhrase(generatedAt: generatedAt, now: now, calendar: calendar)
        let name = partnerName?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        let forWhom = name.isEmpty ? "" : " for \(name)"
        return "These are the picks we found\(forWhom) \(age). Want a fresh set?"
    }
    ...
    func recoverRecentlyStoredBatch(
        ...
        let response: MilestoneRecommendationsResponse
        do {
            // Unscoped on purpose: recovery must see whichever surface's batch
            // is newest, and `isRecoverableBatch` does the milestone matching.
            response = try await milestoneFetcher.fetchLatestRecommendations(justBecause: false)
        } catch {
            return false
        }
