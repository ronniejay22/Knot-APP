...
final class RecommendationsViewModel {
    ...
    /// pre-generated by a push, or recovered. Drives the "Picks from …"
    /// dateline above the feed. `nil` until something has been published.
    ...
    /// The dateline shown above a batch: "Picks from today" / "yesterday" /
    /// "N days ago", by calendar-day difference so a batch from 11pm reads as
    /// "yesterday" at 1am rather than "today". A future timestamp (clock skew)
    /// clamps to "today".
    static func batchAgeLabel(
        generatedAt: Date,
        now: Date = Date(),
        ...
        switch days {
        case 0: return "Picks from today"
        case 1: return "Picks from yesterday"
        default: return "Picks from \(days) days ago"
        }
    }
