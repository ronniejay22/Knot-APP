...
///
/// Dark-themed screen with a vertical list of full-width selectable rows
/// (`InterestSelectRow`) — each showing an SF Symbol icon and the interest name,
/// with an accent border + checkmark when selected. The user must select at least
/// 5 as "likes"; there is no upper bound.
///
/// Features:
/// - Personalized title using the partner's name from Step 3.2
/// - Search bar to filter the interest categories
...
struct OnboardingInterestsView: View {
    ...
    private var headerSection: some View {
        ...
        return OnboardingHeader(
            ...
            subtitle: "Choose at least 5 things your partner enjoys.\nThis helps us personalize gift and date ideas."
        )
        .padding(.top, 4)
    }
